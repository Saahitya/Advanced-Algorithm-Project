...
void read_graph(FILE *fp) {
	...
	for(i = 0; i < N; i++) {
	    ...
	    for(j = 0; j < N; j++){
	        fscanf(fp,"%d",&temp);
	        if(i==j) {gob.distances[i][j] = 0; gob.hops[i][j]=i;}
...
int main(int argc, char *argv[]) 
{
	...
	gob.receiverOccupied=(int*)malloc(sizeof(int)*gob.numnodes);
	...
	thread th0(sendupdate);
	//thread th1(&sendupdate, 1);
	//thread th2(&sendupdate, 2);
